  ...
  return (
    ...
    >
      <div
        ...
        className={`dark:bg-dark_bg_4 w-80 ${
          me ? "right-24" : ""
      ...
      >
        <div>
          <p>
            All{" "}
            <span className="dark:text-gray-400 ml-2">
              {" "}
          ...
          <div className="mt-2">
            <ul>
              {message.reaction.map((react, key) => (
                <li className="pb-2" key={key}>
                  <div className="flex gap-4">
                    <img
                      src={react?.user?.picture}
                      alt=""
                      className="w-8 h-8 rounded-full"
                    />
                    <p>
                      {_id === react?.user?._id ? "You" : react?.user?.name}{" "}
                    </p>
                    <span className="scale-150 absolute right-5">
                      {react.emoji}
                    </span>
                  </div>
                </li>
